    ...
    <Card className="w-full">
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          {book.status === "borrowed" && book.borrowedBy && (
            <p className="text-sm text-muted-foreground">借用者: {book.borrowedBy}</p>
          )}
          {book.dueDate && (
            <p className="text-sm text-muted-foreground flex items-center">
      ...
      <CardFooter>
        {book.status === "available" ? (
          <Button onClick={() => onCheckout?.(book.id)} className="w-full">
            貸出
          </Button>
        ) : (
          <Button onClick={() => onReturn?.(book.id)} variant="outline" className="w-full">
            返却
          </Button>
        )}
      </CardFooter>
