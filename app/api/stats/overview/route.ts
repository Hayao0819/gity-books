...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get available books
    const { count: availableBooks, error: availableBooksError } = await supabaseAdmin
      .from('books')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'available')
      .is('deleted_at', null)
    ...
    // Get borrowed books
    const { count: borrowedBooks, error: borrowedBooksError } = await supabaseAdmin
      .from('books')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'borrowed')
      .is('deleted_at', null)
    ...
    const { count: monthlyCheckouts, error: monthlyCheckoutsError } = await supabaseAdmin
      .from('checkouts')
      .select('*', { count: 'exact', head: true })
    ...
    // Get overdue books
    const { count: overdueBooks, error: overdueBooksError } = await supabaseAdmin
      .from('checkouts')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'borrowed')
